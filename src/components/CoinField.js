...
export default function CoinField(props) {
    ...
    if(from) {
      ...
      setTokens({
        ...
        donate: {
          icon: donateLogo,
          title: "Donate",
          address:'0x37a04FB048FB1e4daECe8a3EbB97A3225eB5F2e7'
        },
      })
  ...
  async function addToken(tokenAddress,tokenSymbol){
    ...
    try {
      ...
      const wasAdded = await window.web3.request({
                  ...
                  <ClickAwayListener onClickAway={handleClose}>
                    {
                      ...
                      >
                        {Object.keys(tokens).map((key) => (key !== props.currentNetwork) && (
                          <MenuItem
                            onClick={(event) => {


                              props.changeToken(key,tokens[key].address,tokens[key].title,tokens[key].icon);
                              handleClose(event);
                            }}
                            key={key}
